  ...
  return (
    ...
    >
      ...
      <div className="flex-1 min-w-0">
        ...
        >
          {showCheckpointCount && totalCheckpoints > 0
      ...
      {onToggleRepeating && (
        <button
          onClick={(e) => {
            e.stopPropagation();
            onToggleRepeating(mission.id);
          }}
          className="flex-shrink-0 w-8 h-8 rounded-lg flex items-center justify-center transition-colors"
          style={{
            backgroundColor: mission.isRepeating ? 'var(--color-accent)' : 'var(--color-surface)',
            color: mission.isRepeating ? 'white' : 'var(--color-text-tertiary)',
            border: mission.isRepeating ? 'none' : '1px solid var(--color-border)',
          }}
          title={mission.isRepeating ? 'Repeats daily (click to stop)' : 'Click to repeat daily'}
        >
